...
const FirebaseAppFromApiKey = function () {
  ...
  useEffect(() => {
    if (!firebaseConfig.apiKey) {
      console.log("********** WARNING **********")
      console.log(
        "You have not set up an API key. We will use the API key passed in via params. This will work, but means"
      )
    ...
    if (!firebaseConfig.apiKey) {
      console.log("********** WARNING **********")
      console.log(
        "You have not set up a project ID. Without this we won't reliably be able to redirect to the default"
      )
      console.log(
        "/auth/action app in case this app is required to perform something other than password reset."
      )
  ...
  //what's passed in.
  const usedFirebaseConfig = firebaseConfig.apiKey
    ? firebaseConfig
